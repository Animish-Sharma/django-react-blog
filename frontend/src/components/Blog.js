...
function Blog() {
    ...
    useEffect(() => {
       const fetchData = async ()=>{
           try {
            const res = await axios.get(`${process.env.REACT_APP_URL}api/blog/featured`);
    ...
    useEffect(() => {
       const fetchData = async ()=>{
           try {
            const res = await axios.get(`${process.env.REACT_APP_URL}api/blog/`);
            console.log(`${process.env.REACT_APP_URL}`)
            setBlogs(res.data);
           }catch(err){
    ...
    const getBlogs = ()=>{
            ...
            return list.push(
                <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
                    <div className="col p-4 d-flex flex-column position-static">
                        <strong className="d-inline-block mb-2 text-primary">{capitalizeLetter(blogPost.category)}</strong>
        ...
        for (let i = 0; i < list.length; i +=2){ //! marked syntax
            result.push(
